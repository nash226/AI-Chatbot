...
def classify_manual(conversation):
    response = llm.responses.create(
        model="gpt-4.1-nano",
        temperature=0,
        input=f"Classify which software product the user is referring to in their final prompt of the following conversation. Your output should be limited to of the following choices: [flamehamster, rumblechirp, verbiage++, guineapigment, emrgency, unsure]. The option of unsure should be used only if you're not certain which software the user is referring to. Here is the conversation: {conversation}"
    )
...
def expand_query(conversation):
    response = llm.responses.create(
        model="gpt-4.1-nano",
        temperature=0,
        input=f"Rewrite, in an expanded way, what the user means to say in their final prompt of the following conversation: {conversation}"
    )
    return response.output_text

def rag(user_input, manual_name, rag_chunks):
    rag_chunks.clear()  # start with a blank dictionary
...
def system_prompt(rag_chunks=None):
    return {"role": "developer", "content": f"""
            ...
            * If the user doesn't mention the name of which GROSS software
            they're asking about, proactively ask them which software they're using. List out the choices.
            ...
            * Do not use your general knowledge to answer a user query. Only use the <documentation> provided above to advise the user.
            * If you cannot find any advice for the user based on the excerpts, simply apologize and say that you do not know how to help the user this time.
            * Before you state any point other than a question, think 
            ...
            <final_instructions>
            * After mentioning any [[citation id]], pause and reflect on the citation id you've cited. Are you about to mention something not found in that citation? YOU ARE INSTRUCTED TO NOT MENTION ANY ADVICE NOT FOUND IN THE DOCUMENTATION!!!
            * If the user suggests something not found in the above <documentation>, you should politely reject the user's point.
            * If your advice does not remain faithful to the <documentation>, I WILL LOSE MY JOB!!! PLEASE REMAIN FAITHFUL!
            </final_instructions>
            """}
...
if __name__ == "__main__":
    ...
    while user_input != "exit":
        ...
        documentation = rag(expanded_query, manual_name, rag_chunks)
