...
# Retrieve the most relevant documentation chunks from Pinecone for the user's question.
def rag(user_input):
...
# Define the chatbot's role and the product domain it is allowed to answer about.
def system_prompt():
...
# Package the retrieved documentation together with the latest user question.
...
# Send the full conversation history to the LLM and return its answer.
...
if __name__ == "__main__":
    # Maintain a running chat history so each turn includes the prior conversation.
